...
每次对数据的修改操作（添加、删除、修改），都会生成一个新的数据版本，同时保留旧版本的数据（其实是通过撤销日志 Undo Log 来恢复旧版本数据，不会真的存储多份，类似 git diff 算法）。

MySQL 默认的【可重复读】隔离级别，通过 MVCC 机制 + 手动加 next-key 锁，有效解决了可重复读和幻读的问题。

MVCC 底层的原理是通过 Undo Log + ReadView 来实现的。
- 多个事务同时修改一份数据，会在内存里保存每个版本。具体是通过 undo log 来实现。
- ReadView 为每个事务创建一个列表，记录了当前活跃的事务和已经提交的事务信息。
- 不同的事务会修改同一条数据，按照先后形成一个历史版本链接，每个事务通过自己的事务列表，找到自己版本的数据。
- 通过 Undo Log + ReadView，每个事务都能自己想要的版本的数据。
...
- 乐观锁是一种编程思想，实际上是不加锁的实现方案。类似 GIT 或者共享文档的方式。
- 乐观锁实现有 CAS 和版本号两种方法。
